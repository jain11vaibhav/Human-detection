...
def person_detection():
    ...
    x = layers.Conv2DTranspose(9,[5, 5],strides=(4,4),padding='same')(x)
...
class MY_Generator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        x=np.zeros((image_shape[0],image_shape[1],9))
#         x= np.full((image_shape[0],image_shape[1],9),255)
	...
	#             channel = i%9
            ...
            cv2.circle(z[:,:,channel],(int(obj[i]['center'][0]),int(obj[i]['center'][1])),
                       int(image_shape[0]/32),(255,255,255),-1)
            img.append(z)
        img=np.array(img)
        output=img
        # print(output)
